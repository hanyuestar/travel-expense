...
CREATE TABLE IF NOT EXISTS site_settings (
  ...
  announce_text TEXT, updated_at INTEGER, updated_by INTEGER
);
...
function routeToJson(row) {
  ...
  const out = { id: row.id, owner_id: row.owner_id, is_seed: !!row.is_seed, year: row.year, name: row.name,
    ...
    scenic: row.scenic, hotel: row.hotel, notes: row.notes, created_at: row.created_at, updated_at: row.updated_at, exp };
  return out;
}
...
function insertRoute(userId, data, isSeed) {
  ...
  db.prepare(`INSERT INTO routes (${ROUTE_COLS}) VALUES (@id,@owner_id,@is_seed,@year,@name,@daterange,@type,@days,@people,@dest,@scenic,@hotel,
...
function updateRoute(id, userId, data) {
  ...
  db.prepare(`UPDATE routes SET year=@year, name=@name, daterange=@daterange, type=@type, days=@days, people=@people,
    dest=@dest, scenic=@scenic, hotel=@hotel, exp_traffic=@exp_traffic, exp_flight=@exp_flight, exp_train=@exp_train,
