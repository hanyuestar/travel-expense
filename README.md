...
- **修复**：路线概览「预算」行在**超支**时错误地显示出原始 HTML 标签（如 `<span …>超支 ¥…</span>`），现改为正常的**红色「超支 ¥X」**显示；多用户版与单用户演示版同步修复。
- **修复**：连续编辑不同路线时，景点路线 / 住宿输入框可能**显示为空或串入上一条路线的内容**（现改为「打开路线一律以该路线数据为准并重置输入框，仅改日期时保留当前输入」）。
- **清理**：移除 4 处未被调用的函数、若干未使用的 CSS 规则与变量、以及多余的 `export` 关键字（**不改动任何行为**）。
