...
/* CATS / esc / fmtMoney 统一从 api.js 引入（单一来源），不在本文件重复定义 */
import { fmtMoney, CATS, esc } from './api.js';
...
export function donut(data, cur) {
  ...
  return `<svg viewBox="0 0 180 180" class="chart">${segs}
    <text x="90" y="86" text-anchor="middle" class="donut-total">${fmtMoney(total, cur)}</text>
    <text x="90" y="106" text-anchor="middle" class="donut-sub">总花费</text></svg>`;
}

export function trendBar(points, xLabelKey, cur) {
  ...
  points.forEach((p, i) => {
    ...
    s += `<rect x="${x}" y="${y0}" width="${bw}" height="${h}" rx="6" fill="${COLORS['机票']}"
      opacity="${(0.55 + 0.45 * (p.value / max)).toFixed(2)}"><title>${esc(xLabelKey ? p[xLabelKey] : p.label)}：${fmtMoney(p.value, cur)}</title></rect>`;
    s += `<text x="${x + bw / 2}" y="${y0 - 6}" text-anchor="middle" font-size="12" fill="#1f2d2b" font-weight="700">${fmtMoney(p.value, cur)}</text>`;
